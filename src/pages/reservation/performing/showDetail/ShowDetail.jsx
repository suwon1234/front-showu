...
const ShowDetail = () => {
 ...
 const handleDateChange = (date) => {
   const dateObj = new Date(date.getTime() - date.getTimezoneOffset() * 60000); // 로컬 타임존을 UTC로 변환
        ...
        {dateArray.map((date) => {
          ...
          return (
            <S.CalendarDay
              key={date}
              onClick={() =>
                !isBefore(localDate, today) && handleDateChange(localDate)
              } // 현재 날짜 이전의 날짜는 선택 불가
              selected={isSameDay(localDate, selectedDate)}
            >
              {format(localDate, "d")}
            </S.CalendarDay>
          );
